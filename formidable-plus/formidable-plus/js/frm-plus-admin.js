jQuery(function($){
 ...
 $(".frm-table-container br").remove();


 //$('.frmplus_field_type').select2();
 
 
//  $(document).ready(function() {
//   // Remove the option with value "data_from_entries"
//   $(".frmplus_field_type option[value='data_from_entries']").remove();
// });
	...
	$.extend( FRMPLUS, {
		init: function(){
			$('#frm_form_editor_container')
				...
				.on('change','.frm_ipe_table_field input',function(){
					$.post(ajaxurl,{
				...
				}).on('click','.frm_delete_field_col,.frm_delete_field_row',function(){
				    var cont = jQuery(this).parent('.frm_single_option').attr('id'); 
					var matches = $(this).closest('.frm_single_option').attr('id').match( /frm_delete_field_([0-9]+)-(.+)_container/ ),
						field_id = matches[1],
						opt_key = matches[2];

					$.post(ajaxurl,{
						action: 'frm_delete_field_option',
						field_id: field_id,
						opt_key: opt_key,
				// 		newadded
