	...
	public function get_data_values( $options, $entry_id = null ){
		...
		if ( isset( $cache ) ){
			...
			if ( 'taxonomy' === $options['source']['form'] ){
				...
				if ( isset( $entry_id ) ){
					$values = FrmProDynamicFieldsController::get_independent_options( $values, $field, $entry_id );
				}
				else{
					$values = FrmProDynamicFieldsController::get_independent_options( $values, $field );
				}
			}
