	...
	function massage_request($html, $field, $meta, $unmassage = false){
        ...
        if (isset($matches[0]) and !empty($matches[0])){
			if (!isset($table_field_ids)){
				$result = FrmField::getAll('fi.type = "table"');
...
if (!isset($the_sections[$field_id])) {
    ...
    $the_sections[$field_id] = $wpdb->get_var(
        $wpdb->prepare(
            ...
            SELECT
                the_section.id
            FROM
                {$wpdb->prefix}frm_fields AS the_table
            LEFT JOIN
                {$wpdb->prefix}frm_forms AS the_table_form ON the_table_form.id = the_table.form_id
            RIGHT JOIN
                {$wpdb->prefix}frm_fields AS the_section ON the_section.form_id = the_table_form.parent_form_id
            WHERE
                the_table.id = %d
                AND the_table.type = 'table'
                AND the_section.type = 'divider'
    ...
    // Setting to false so it will pass isset()
    if (!isset($the_sections[$field_id])) {
        $the_sections[$field_id] = false;
    }
}
