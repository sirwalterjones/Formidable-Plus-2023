...
<?php 
//var_dump($field['custom_row_name_list']);
if($field['custom_column_name_list'] != NULL){
$stringsmart = $field['custom_column_name_list'];
}else{
$stringsmart = $bb;
}
$values = explode(',', $stringsmart);
$column = substr(FrmPlusFieldsHelper::parse_option($opt,'name'), 7);
$words = explode(' ', FrmPlusFieldsHelper::parse_option($opt,'name')); // Split the string into an array of words
...
//var_dump($field);
?>
<th class="column-<?php echo $col_num++; ?>"><?php echo $result; ?></th>
<?php endforeach; ?>
<?php if (!count($rows)) : ?>
